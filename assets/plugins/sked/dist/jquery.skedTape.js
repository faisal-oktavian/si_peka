...
SkedTape.defaultFormatters = {
	...
	duration: function (ms, opts) {
		...
		var hrs = (opts && opts.hrs) || ' jam';
		var min = (opts && opts.min) || ' menit';
		var format = h ? h + hrs : '';
		format += h && m ? ' ' : '';
		format += m ? m + min : '';
		return format;
...
SkedTape.prototype = {
	...
	renderDates: function() {
		...
		var arr_month = ['Januari', 'Febuari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember'];

		queue.forEach(function(item) {
			var proportion = item.weight / totalWeight;
			var text = new Date(item.text);
			// var title = text.getDate() + "-" + (text.getMonth() + 1) + "-" + text.getFullYear();
			var title = text.getDate() + " " + (arr_month[text.getMonth()]) + " " + text.getFullYear();
			$('<li/>')
